...
void thomas_init(void) {
  int i;
  SDL_Surface * t;
  int vp[4], w, h;
  static char * files[] = {"images/anim4/thomas.png","images/anim4/eye.jpg"};
  _pId  = gl4duCreateProgram("<vs>shaders/thomas.vs", "<fs>shaders/thomas.fs", NULL);
  ...
  void thomas_draw(void) {
  static GLfloat a0 = 0.0;
  static GLfloat dance = 0.0;
 static int condi1 =0;
...
///////////////////////////////////////////////////
  ...
  gl4duPushMatrix(); {
    gl4duTranslatef(0, 0+dance, -20);  
    gl4duScalef(9, 8, 0.01);
    gl4duSendMatrices();
  }gl4duPopMatrix();
...
//OEIL 1
    gl4duPushMatrix(); {
            gl4duTranslatef(3,6+dance,-19.5);  
        glBindTexture(GL_TEXTURE_2D, _tId[1]);
        gl4duPushMatrix(); {
            gl4duRotatef(a0, 1, 0, 1);
